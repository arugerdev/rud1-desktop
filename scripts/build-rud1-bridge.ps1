...
.NOTES
  ...
  El módulo rud1-bridge está vendorizado en <repo-root>/native/rud1-bridge.
#>
param(
  [switch]$Force,
  [string]$Version = "dev"
...
$BridgeRoot   = Resolve-Path (Join-Path $RepoRoot "native\rud1-bridge")
$ResourceRoot = Join-Path $RepoRoot "resources"

if (-not (Test-Path $BridgeRoot)) {
    throw "rud1-bridge module not found at $BridgeRoot. Vendorizado en native/rud1-bridge."
}
