...
/**
 * Instala com0com en silencio desde el instalador firmado empaquetado.
 * La app es requireAdministrator → `setup.exe /S` corre elevado sin UAC
 * extra ni MOTW/SmartScreen (lo lanzamos nosotros). El driver va firmado
 * por catálogo (com0com.cat). Ver docs/serial-com0com-migration.md §4.
 */
async function installCom0comSilently(): Promise<void> {
  const installer = com0comInstallerPath();
  if (!installer) {
    throw new Error(
      "com0com installer no empaquetado. Ejecuta `npm run fetch:com0com-win` " +
...
/**
 * Crea un par com0com nuevo (CNCAx/CNCBx) ya con alias COMxx + EmuBR, de
 * forma que Arduino IDE lo enumere directamente. Usado cuando tras
 * instalar no hay ningún par utilizable.
 */
...
/**
 * Garantiza que com0com está instalado Y tiene un par usable
 * (aliased + EmuBR), provisionando todo en silencio si falta. Devuelve
 * el par listo para usar. Es la pieza que cumple el requisito de
 * "instala y administra el driver solo, sin que el usuario haga nada".
 *
 * Si la auto-provisión falla en algún paso, deja propagar el error
 * tipado (Com0comMissingError / *NotAliased / *NoEmuBR) para que el
 * panel pinte el CTA manual como último recurso.
 */
export async function ensureCom0comReady(): Promise<Com0comPair> {
  if (process.platform !== "win32") {
    throw new Error("ensureCom0comReady is Windows-only");
  }
  let status = await detectCom0com();

  // 1) Driver: instalar en silencio si falta.
  if (!status.installed) {
    await installCom0comSilently();
    // El registro del driver + setupc tarda un instante en aparecer.
    await new Promise((r) => setTimeout(r, 1500));
    status = await detectCom0com();
    if (!status.installed) throw new Com0comMissingError(status);
  }

  // 2) Par usable: si ya hay uno aliased+EmuBR, listo.
  const ready = pickPair(status);
  if (ready && ready.hasComAlias && ready.emuBR) return ready;

  // 3) Provisionar: sin pares → crear uno; con pares sin alias/EmuBR →
  //    reconfigurar el existente.
  if (status.pairs.length === 0) {
    if (!status.setupcPath) throw new Com0comMissingError(status);
    await installCom0comPair(status.setupcPath, status);
  } else {
    await serialBridgeConfigurePair();
  }

  await new Promise((r) => setTimeout(r, 500));
  status = await detectCom0com();
  const usable = pickPair(status);
  if (!usable || !usable.hasComAlias) throw new Com0comMissingError(status);
  if (!usable.emuBR) throw new Com0comPairNoEmuBRError(usable, status.setupcPath);
  return usable;
}
...
export async function serialBridgeOpen(opts: OpenOptions): Promise<OpenResult> {
  ...
  if (!isRud1BridgeAvailable()) {
    throw new Error(
      "rud1-bridge binary missing. Run `npm run build:rud1-bridge` to compile " +
  ...
  let linkArg: string[] = [];
  if (process.platform === "win32") {
    // Auto-instala el driver y provisiona el par (alias + EmuBR) en
    // silencio si falta — el operador no tiene que tocar setupc. Solo
    // si la auto-provisión no lo deja listo se propagan los errores-CTA
    // (Com0comMissingError / *NotAliased / *NoEmuBR) para el panel.
    const picked = await ensureCom0comReady();
    pair = picked;
